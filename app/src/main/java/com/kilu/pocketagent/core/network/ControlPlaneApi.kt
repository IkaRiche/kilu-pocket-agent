...
class ControlPlaneApi(
    ...
    // ── E3.2 Phase B: Workflow Grant ──────────────────────────────────────────────

    /**
     * GET /v1/workflow-grants/:id
     * Returns WorkflowGrantDetail (unwrapped from envelope if present).
     * Returns null on network or auth failure.
     */
    suspend fun getWorkflowGrant(grantId: String): WorkflowGrantDetail? =
        withContext(Dispatchers.IO) {
            try {
                val req = Request.Builder().url("$baseUrl/workflow-grants/$grantId").get().build()
                client.newCall(req).execute().use { resp ->
                    when {
                        resp.isSuccessful -> {
                            val body = resp.body?.string() ?: return@withContext null
                            try {
                                json.decodeFromString<WorkflowGrantDetailResp>(body).unwrap()
                            } catch (_: Exception) {
                                try { json.decodeFromString<WorkflowGrantDetail>(body) }
                                catch (_: Exception) { null }
                            }
                        }
                        resp.code == 401 || resp.code == 403 -> { onAuthFailure?.invoke(); null }
                        else -> { logger.e("ControlPlaneApi", "getWorkflowGrant http=${resp.code}"); null }
                    }
                }
            } catch (e: Exception) {
                logger.e("ControlPlaneApi", "getWorkflowGrant error", e)
                null
            }
        }

    /**
     * POST /v1/workflow-grants/:id/approve
     *
     * One-tap APPROVE ALL: transitions all PLANNING tasks to READY_FOR_EXECUTION atomically.
     * Biometric gate must be passed before calling this — same as per-step approvePlan.
     *
     * Returns Result.success(ApproveWorkflowGrantResp) on success.
     * Returns Result.failure(Exception) on error — message is display-safe.
     */
    suspend fun approveWorkflowGrant(
        grantId: String,
        req: ApproveWorkflowGrantReq,
    ): Result<ApproveWorkflowGrantResp> =
        withContext(Dispatchers.IO) {
            try {
                val body = json.encodeToString(req).toByteArray().toRequestBody(mediaTypeJson)
                val httpReq = Request.Builder()
                    .url("$baseUrl/workflow-grants/$grantId/approve")
                    .post(body)
                    .build()
                client.newCall(httpReq).execute().use { resp ->
                    ...
                    when {
                        resp.isSuccessful ->
                            Result.success(json.decodeFromString<ApproveWorkflowGrantResp>(bodyStr))
                        resp.code == 401 || resp.code == 403 -> {
                            onAuthFailure?.invoke()
                            Result.failure(Exception(parseErrorCode(bodyStr, resp.code)))
                        }
                        else -> Result.failure(Exception(parseErrorCode(bodyStr, resp.code)))
                    }
                }
            } catch (e: Exception) {
                logger.e("ControlPlaneApi", "approveWorkflowGrant error", e)
                Result.failure(e)
            }
        }

    private fun parseErrorCode(body: String, httpCode: Int): String = try {
        val obj = json.parseToJsonElement(body)
        val err = obj.jsonObject["error"]?.jsonObject
        val code = err?.get("code")?.jsonPrimitive?.content ?: "ERR_UNKNOWN"
        val msg = err?.get("message")?.jsonPrimitive?.content ?: body.take(200)
        "[$code] $msg"
    } catch (_: Exception) { "[HTTP $httpCode] ${body.take(200)}" }
